...
function contruirTemplate(nombre, numero, id) {
	...
	var textoTelefono = document.createTextNode(numero);
	...
	inputTelefono.name = 'telefono_' + id;
  	...
  	xhr.onreadystatechange = function() {
	  	if(xhr.readyState == 4 && xhr.status == 200){
	  		...
	  		if (json.Respuesta == true) {
	  			...
	  			contruirTemplate(json.nombre, json.numero, json.id);
	  		}
  		}
  	}
...
function ocultarRegistros(nombre_buscar) {
	// variables para todos los registros
	var registros = tableBody[0].getElementsByTagName('tr');
	// expresion regular que busca el nomvre con case insensitive
...
function actualizarRegistro(idRegistro) {
	// Seleecionar Boton de Guardar del Registro en especifico(se pasa el ID)
